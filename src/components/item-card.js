...
const ItemCard = ({img, name, title, bid}) => {
    return (
        <div className='border  border-b border-border   drop-shadow rounded-lg p-4 xl:w-[300px] mx-auto'>
            <div className='bg-rayLight rounded-lg'>
                <img alt='laptop' src={img} />
            </div>
            <div className='flex items-center my-3'>
                <div className='flex mr-2 justify-center items-center w-[40px] h-[40px] bg-[#F2F4F7] rounded-[50%] text-radDark2'> KO</div>
                <p><span className='text-md'>{name}<span className='font-light text-[#98A2B3] text-sm'>(Highest Bidder)</span></span></p>
            </div>
            <p className='my-2 font-semibold text-md'>{title}</p>
            <p className='my-2'><span className='text-md capitalize text-[#344054]'>current Bid: <span className='font-semibold text-sm'>₦{bid}</span></span></p>
